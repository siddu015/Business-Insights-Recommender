...
ENGINEERED_CSV = os.path.join(DATA_DIR, "engineered_business_data.csv")
LABELS_JSONL = os.path.join(DATA_DIR, "training_labels.jsonl")
...
def load_dataset() -> List[Dict]:
    ...
    df = df.drop_duplicates(subset=["Business_ID"])  # safety

    # Build lookup by business_id for fast joins
    ...
    with open(LABELS_JSONL, "r") as f:
        for line in f:
            ...
            # target is fully structured JSON
            target_obj = {
                "business_id": biz_id,
...
def data_collator(features: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    """
    Collate batch of features into padded tensors.
    Returns a dictionary (not a custom dataclass) to be compatible with Trainer.
    """
...
def main():
    ...
    # Reasonable defaults for laptop training
