...
LOCATIONS = [
    ...
    "São Paulo, Brazil", "Mexico City, Mexico", "Lagos, Nigeria", "Cairo, Egypt"
]
...
Example format: [["Challenge 1", "Challenge 2", "Challenge 3"], ["Challenge A", "Challenge B", "Challenge C"], ...]"""
    ...
    def generate_market_data_bulk(self):
        """Generate market data efficiently"""
        print("Generating market data...")
        
        market_prompt = f"""Generate market data for 30 different location-industry combinations. Include major cities and common industries. Output as JSON array with these exact fields:

- Location: City, State/Province, Country
- Industry: One of {INDUSTRIES}
- Consumer_Spending_Growth_YoY: Number between -10 and 15
- Competition_Density_Score: Integer 1-10
- Regulatory_Changes_Upcoming: 2-3 sentence description
- Digital_Adoption_Rate: Number 0-100
- Local_Economic_Health_Score: Integer 1-10
- Industry_Growth_Trend: "Growing", "Stable", or "Declining"

Output exactly 30 market data objects as a JSON array."""

        try:
            response = model.generate_content(market_prompt)
            market_data = self.parse_json_array(response.text)
            if not market_data:
                market_data = self.create_fallback_market_data()
            return market_data
        except:
    ...
    def save_templates(self):
        ...
        with open('/Users/siddu/Developer/Business-Insights-Recommender/generated_templates.json', 'w') as f:
            json.dump(templates_data, f, indent=2)
        
        print(f"Saved templates to generated_templates.json")
    ...
    def save_datasets(self, businesses: List[Dict], market_data: List[Dict]):
        ...
        business_df.to_csv('/Users/siddu/Developer/Business-Insights-Recommender/business_data.csv', index=False)
        print(f"Saved {len(business_df)} business records to business_data.csv")
        
        market_df = pd.DataFrame(market_data)
        market_df.to_csv('/Users/siddu/Developer/Business-Insights-Recommender/market_data.csv', index=False)
        print(f"Saved {len(market_df)} market records to market_data.csv")
